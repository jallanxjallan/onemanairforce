...
def action(elem, doc):
    if isinstance(elem, pf.Span) and elem.content[0].text == 'METADATA':
        atts = elem.attributes
        date = atts.get('date', None) 
        note = atts.get('note_content', None) 
        if date:
            doc.sluglines.append(Slugline(date, atts['category'])) 
        elif note:
            return pf.Span(pf.Strong(pf.Str('Content Note:')), pf.Space)
        else:
            msg = f'No Slugline Data for {atts["title"]}'
            return pf.Span(pf.Strong(pf.Str(msg)), pf.Space)
