...
class ScreenplayData():
    ...
    def __init__(self):
        ...
        incidents = pd.DataFrame([load_incident(n, ct)
                   for c in CATEGORIES
                   for n in ct.nodes(c)
                   if n.level > 2
                   if not n.name.startswith("~")])

        scenes = pd.DataFrame([load_scene(no, n, ct) for no, n in enumerate(ct.nodes('Scenes'))])
    ...
    def filter_by_keyword(self, negate=False, **kwargs):
        ...
        clauses = [f'({prefix}{f.lower()}.str.contains("{a}"))' for f,a in kwargs.items()]
        query = ' & '.join(clauses)
    ...
    def filter_by_existence(self, **kwargs):
        ...
        clauses = [f'({f.lower()}.{"notna()" if a else "isna()"})' for f,a in kwargs.items()]
        query = ' & '.join(clauses)
    ...
    def display_content(self):
        for r in self.df.dropna(subset=['content']).sort_values(['scene_no', 'story_no']).itertuples():
            try:
                display(
